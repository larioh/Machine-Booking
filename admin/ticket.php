	 ...
	 <div class="container">
        ...
        <div class="panel-body"style="background-color:#90EE90;margin-top: 0%;padding-bottom: 5px;border-radius: 15px 15px 15px 15px;height: 100%;width: 100%;">
             <div class="panel-heading">
                <h2 class="panel-title" style="text-align: center;font-size: 30px;"><b> Provide Tenant details below <b></h2><br><br>
                    </div>
...
<!-- <form action="index.php" method="post"> -->
 ...
 <div class="form-group has-feedback">
        <label class="col-sm-3 control-label">Rental Name</label>
        <div class="col-sm-6"> 
          <?php
               require('dbconn.php');
                 $sql = "SELECT *  FROM vehicle order by name";
               //$sql =  "select vehicle.car_no,route_fare from vehicle order by car_no";
                $result = $conn->query($sql);
                if ($result->num_rows > 0) {
                    echo "<select name='name' id='name' class='select form-control' required>";
              ...
              ?>
        <!-- <select name="troute" id="troute" class="select form-control" required=""><option value="">Select Route</option></select> -->
        </div>
    </div>
                                <!-- route -->
    <div class="form-group has-feedback">
        <label class="col-sm-3 control-label">Locatio</label>
        <div class="col-sm-6"> 
        <select name="location" id="location" class="select form-control" required=""><option value="">Select Name First</option></select>
        </div>
    </div>
                                <!-- end route -->
...
<div class="form-group">
      ...
      <div class="col-sm-6">
        <input type="number" class="form-control" name="amount"  placeholder="500"  required="true">
      </div>
  </div>
  <div class="form-group has-feedback">
        <label class="col-sm-3 control-label">Room number</label>
        <div class="col-sm-6"> 
        <select name="rooms" id="rooms" class="select form-control" required=""><option value="">Select Location First</option></select>
        </div>
    </div>  
  <div class="form-group">
  ...
  <button type="submit" class="btn btn-success btn-block"   name="submit">Save</button>
   ...
   <a href="inde.php" class="btn btn-success" role="button" >View Rental Booked</a>
...
require('dbconn.php');
 ...
 if(isset($_POST['submit'])){
  ...
  $code=$_POST['tcode'];
   ...
   $query="INSERT INTO  `houses` .`ticket` (name,`pname`,`idno`,`location`,`phone`,`seats`,`code`,`amount`) VALUES('$name','$pname','$idno','$location','$phone','$seats','$code','$amount')";


if(mysqli_query($conn,$query)==TRUE) {
  echo "Rental succesfully booked";
    // redirects to view
   }
   else
   {
